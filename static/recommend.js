$(function() {
    ...
    $(".movie-button").on("click", function() {
        var my_api_key = "144f5df1b99e71d3d4b0422cce695dc9";
        var title = $(".movie").val();
        if (title == "") {
...
async function getURL(imdb_id) {
    const data = await fetch(
            `https://imdb-api.com/en/API/YouTubeTrailer/k_wjv0o939/${imdb_id}`
    );
...
// passing all the details to python's flask for displaying and scraping the movie reviews using imdb id
 async function show_details(movie_details, arr, movie_title, my_api_key, movie_id) {
    ...
    const trailerURL=await getURL(imdb_id)
